			...
			<div class="col-lg-12 m-lr-auto m-b-50">
				...
				<table class="table">
					...
					<tr v-for="vo in notice_list">
						<td class="text-left wd-50"><a :href="'notice_detail.do?no=' + vo.n_id">{{vo.n_title}}</a></td>
				...
				<div class="text-right p-t-10">
					<button class="btn btn-sm bg-2" v-on:click="insert()">글쓰기</button>
				</div>
				</c:if>
				<div class="text-center p-t-20 p-b-20">
       			 <ul class="pagination">
       			   <li class="page-item" :class="{'disabled':start==1}"><button class="page-link" :value="start-1" 
       			    @click="paging($event)"><i class="fa fa-chevron-left" aria-hidden="true"></i></button></li>
       			   <li class="page-item" v-for="i in pages" :class="{'active':i==curpage}"><button class="page-link" :value="i"
       			    @click="paging($event)">{{i}}</button></li>
       			   <li class="page-item" :class="{'disabled':end==totalpage}"><button class="page-link" :value="end+1"
       			    @click="paging($event)"><i class="fa fa-chevron-right" aria-hidden="true"></i></button></li>
      			  </ul>
				</div>
			</div>
		</div>
