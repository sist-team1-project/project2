...
public interface UserMapper {
	...
	// 회원가입: id 존재 여부
	@Select("SELECT COUNT(*) FROM users " + "WHERE u_id=#{id}")
	public int idCount(String id);

	// 회원가입: email 존재 여부
	@Select("SELECT COUNT(*) FROM users " + "WHERE u_email=#{email}")
	public int emailCount(String email);

	// password, name
	@Select("SELECT u_password||','||u_grade FROM users " + "WHERE u_id=#{id}")
	...
	// 아이디 찾기: 이름 & 이메일 존재 여부
	@Select("SELECT COUNT(*) FROM users WHERE u_name=#{name} AND u_email=#{email}")
	public int nameEmailCount(Map map);

	// 아이디 찾기: 아이디 결과
	@Select("SELECT RPAD(SUBSTR(u_id,1,4),LENGTH(u_id),'*') FROM users WHERE u_name=#{name} AND u_email=#{email}")
	public String idFind(Map map);

	// 회원정보 수정
	@Update("UPDATE users SET "
			+ "u_phone=#{u_phone}, u_email=#{u_email}, u_post=#{u_post}, u_address1=#{u_address1}, u_address2=#{u_address2}, u_question=#{u_question}, u_answer=#{u_answer} "
			+ "WHERE u_id=#{u_id}")
	public void userUpdate(UserVO vo);

	// 비밀번호 찾기: 아이디 & 이메일 존재 여부
	@Select("SELECT COUNT(*) FROM users WHERE u_id=#{id} AND u_email=#{email}")
	public int idEmailCount(Map map);

	// 비밀번호 찾기: 질문 가져오기
	@Select("SELECT u_question FROM users WHERE u_id=#{id} AND u_email=#{email}")
	public String questionFind(Map map);

	// 비밀번호 찾기: 질문 & 답변 일치 여부
	@Select("SELECT COUNT(*) FROM users WHERE u_id=#{id} AND u_question=#{question} AND u_answer=#{answer}")
	public int questionAnswerCount(Map map);

	// 비밀번호 수정: 비밀번호 재확인
	@Select("SELECT u_password FROM users WHERE u_id=#{id}")
	public String userGetPassword(String id);

	// 비밀번호 변경
	@Update("UPDATE users SET u_password=#{newPassword} WHERE u_id=#{uid}")
	...
	/* 페이징 사용 */
	@Select("SELECT CEIL(COUNT(*) / 10.0) " + "FROM users ")
