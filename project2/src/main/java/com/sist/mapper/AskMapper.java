...
public interface AskMapper {
	...
	@Update("UPDATE ask_1 SET "
			+"a_visits=a_visits+1 "
			+"WHERE a_id=#{a_id}")
	public void askVisitsIncrement(int no);

	@Select("SELECT a_id,u_id,a_type,a_content,"
			+"TO_CHAR(a_regdate,'YYYY-MM-DD') as a_regdate,a_visits,a_group_id,a_group_step "
			+"FROM ask_1 "
			+"WHERE a_id=#{a_id}")
	...
	@Insert("INSERT INTO ask_1(a_id,u_id,a_type,a_content,a_pwd,"
			+"a_group_id,a_group_step,a_group_tab) "
			+"VALUES(ask_id_seq_1.nextval,"
			+"#{u_id},#{a_type},#{a_content},#{a_pwd},"
			+"#{a_group_id},#{a_group_step},#{a_group_tab})")
	public void askReplyInsert(AskVO vo);

	@Select("SELECT a_pwd FROM ask_1 "
			+"WHERE a_id=#{a_id}")
	public String askGetPassword(int no);
